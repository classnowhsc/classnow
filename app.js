// ============================================================
// ClassNow — App + Firebase Authentication
// ============================================================
...
function waitForAuthState() {
  ...
  if (authStateReady) return Promise.resolve(firebaseAuth.currentUser);
...
const Auth = {
  ...
  async register(name, email, password) {
    if (!initFirebase()) return { ok: false, msg: 'Firebase could not be initialized.' };
    try {
      ...
      return { ok: false, msg: firebaseAuthError(error) };
    }
  },
...
async function checkSessionValidity() {
  ...
  if (window.location.pathname.includes('classes.html')) {
    showToast('Please login to access your classes.', 'error');
...
function initNavAuth() {
  ...
  if (userNameEl && loggedIn) userNameEl.textContent = firstName;

  if (mobAuth && mobUser) {
    mobAuth.style.display = loggedIn ? 'none' : 'flex';
    mobAuth.style.flexDirection = 'column';
    mobAuth.style.gap = '4px';
    mobUser.style.display = loggedIn ? 'flex' : 'none';
    if (mobName && loggedIn) mobName.textContent = firstName;
  }
}
...
function initParticles() {
  ...
  const draw = () => {
    ...
    pts.forEach((p, i) => {
      ...
      ctx.beginPath();
      ctx.arc(p.x, p.y, p.r * (.5 + p.z * .5), 0, Math.PI * 2);
      ctx.fillStyle = `rgba(0,198,255,${.08 + p.z * .22})`;
      ctx.fill();
      ...
      for (let j = i + 1; j < pts.length; j++) {
        ...
        if (d < 120) {
          ...
          ctx.moveTo(p.x, p.y);
          ctx.lineTo(q.x, q.y);
          ctx.strokeStyle = `rgba(0,198,255,${(1 - d / 120) * .06})`;
          ctx.lineWidth = .5;
          ctx.stroke();
        }
      }
...
function initTiltCards() {
  ...
  document.querySelectorAll('.tilt-card').forEach(card => {
    ...
    card.addEventListener('mousemove', e => {
      ...
      if (shine) {
        shine.style.background = `radial-gradient(circle at ${((e.clientX - r.left) / r.width) * 100}% ${((e.clientY - r.top) / r.height) * 100}%, rgba(255,255,255,0.1), transparent 60%)`;
...
function renderCourses() {
  ...
  grid.innerHTML = COURSES.map((c, i) => {
    if (c.isFRB) return `
      <button class="course-card frb-card tilt-card reveal reveal-delay-1"
